...
Reverse proxy and API gateway sitting in front of internal Smolla services. Built on .NET 10 with YARP.

## Repository layout

```
backend/
    src/Smolla.Gateway.Host/   ASP.NET Core web host
    tests/Smolla.Gateway.Tests/ xUnit unit + integration tests
```
...
dotnet build
dotnet test
dotnet run --project src/Smolla.Gateway.Host
```
